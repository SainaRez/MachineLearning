...
def compute_z(x,w,b):
    '''
        Computes the linear logit value of a data instance. z = <w, x> + b  <-- in this program, we use w and b instead of theta which includes both of them.
...
def compute_L(a,y):
    '''
        Computes the loss function: the negative log likelihood, which is the negative logarithm of the likelihood. 
...
def forward(x,y,w,b):
    '''
       Forward pass: given an instance in the training data, computes the logit z, activation a and cross entropy L on the instance. 
        Input:
...
def compute_dL_da(a, y):
    '''
        Computes local gradient of the cross-entropy function (the Loss function) L w.r.t. the activation a.
        Input:
            a: the activation value, a float scalar
            y: the label of a training instance, an integer scalar value. The values can be 0 or 1.
        Output:
...
def compute_da_dz(a):
    '''
        Computes local gradient of the sigmoid activation a w.r.t. the logit z.
        Input:
...
def compute_dz_dw(x):
    '''
        Computes partial gradients of the logit function z with respect to (w.r.t.) the weights w. 
        Input:
            x: the feature vector of a data instance, a float numpy matrix of shape p by 1. 
...
def compute_dz_db():
    '''
        Computes partial gradient of the logit function z with respect to (w.r.t.) the bias b. 
        Output:
...
def backward(x,y,a):
    '''
       Back Propagation: given an instance in the training data, computes the local gradients for logit, activation, weights and bias on the instance. 
        Input:
            x: the feature vector of a data instance, a float numpy matrix of shape p by 1. 
            y: the label of a training instance, an integer scalar value. The values can be 0 or 1.
            a: the activation, a float scalar
...
def compute_dL_dw(dL_da, da_dz, dz_dw):
    '''
       Given local gradients, computes the gradient of the loss function L w.r.t. the weights w.
        Input:
            dL_da: the local gradient of the loss function w.r.t. the activation, a float scalar value.
            da_dz: the local gradient of the activation a w.r.t. the logit z, a float scalar value. It represents ( d_a / d_z )
            dz_dw: the partial gradient of logit z with respect to the weight vector, a numpy float matrix of shape (p by 1). 
...
def compute_dL_db(dL_da, da_dz, dz_db):
    '''
       Given the local gradients, computes the gradient of the loss function L w.r.t. bias b.
        Input:
            dL_da: the local gradient of the loss function w.r.t. the activation, a float scalar value.
            da_dz: the local gradient of the activation a w.r.t. the logit z, a float scalar value. It represents ( d_a / d_z )
            dz_db: the partial gradient of logit z with respect to the bias, a float scalar. It represents (d_z / d_b).
...
def update_w(w, dL_dw, alpha=0.001):
    '''
       Given an instance in the training data, updates the weights w using gradient descent.
        Input:
...
def update_b(b, dL_db, alpha=0.001):
    '''
       Given an instance in the training data, updates the bias b using gradient descent.
        Input:
...
def train(X, Y, alpha=0.001, n_epoch=100):
    '''
        Given a training dataset, trains the logistic regression model by iteratively updating the weights w and bias b using the gradients computed over each data instance. 
    ...
    # initializes weights and biases as 0
...
def predict(Xtest, w, b):
    '''
       Predicts the labels of the instances in a test dataset using logistic regression.
        Input:
    ...
    '''
    n = Xtest.shape[0]
    Y = np.zeros(n) # initializes as all zeros
...
def check_dL_da(a, y, delta=1e-7):
    '''
        Computes local gradient of the cross-entropy function w.r.t. the activation using gradient checking.
        Input:
            a: the activation value, a float scalar
            y: the label of a training instance, an integer scalar value. The values can be 0 or 1.
...
def check_dz_dw(x,w, b, delta=1e-7):
    '''
        computes the partial gradients of the logit function z w.r.t. weights w using gradient checking.
...
def check_dz_db(x,w, b, delta=1e-7):
    '''
        computes the partial gradients of the logit function z w.r.t. the bias b using gradient checking.
        The idea is to add a small number to the weights and b separately, and approximate the true gradient using numerical gradient.
...
def check_dL_dw(x,y,w,b, delta=1e-7):
    '''
       Given an instance in the training data, computes the gradient of the weights w using gradient check.
        Input:
...
def check_dL_db(x,y,w,b, delta=1e-7):
    '''
       Given an instance in the training data, computes the gradient of the bias b using gradient check.
        Input:
            x: the feature vector of a training instance, a float numpy vector of length p. Here p is the number of features/dimensions.
            y: the label of a training instance, an integer scalar value. The values can be 0 or 1.
            w: the weight vector, a float numpy vector of length p.
